...
> not, and a few of the files named below have never been written.
>
> What the app actually does is in [docs/FEATURES.md](docs/FEATURES.md).
> What shipped and when is in [docs/EVOLUTIONS.md](docs/EVOLUTIONS.md).
>
> Every entry carries a status:
> **DESIGNED** nothing built yet · **PARTIAL** some of it exists · **SHIPPED in vNN**.
> A file named `(not yet written)` is a file that does not exist in this tree.
>
> **Every user-facing line invented here is flagged for the owner** and lives in
> `config/` so it can be reworded without touching code.

Each item: what was imagined, why, and where it would live. **Every user-facing line invented here is flagged for the owner** and lives in `config/` so it can be reworded without touching code.

1. **The flora**: eight original morphologies across twelve forms, one grammar. See `FLORA.md` (not yet written). *Why this way:* one set of primitives (filament, membrane, pod, lens) parameterized by family keeps the eight organisms recognizably one species of light while giving each a signature behavior a person can name in a sentence.
...
6. **Twelve more recipes** (`config/recipes.json`, `imagined: true`): weather · hands · sound · enough · borrowed · almost · stranger · ordinary · setdown · taught · tomorrow · protect, each with a why. **Sixty more lineages** (`config/lineage.json` (not yet written), `imagined: true`) across school, travel, grief, recovery, faith, sport, parenting, city life, seasons, each reason under eight words, every one moving toward light, connection, or ground. **Eight more marks** (`config/rewards.json` (not yet written), `imagined: true`) that reward practice, never volume: Two Touches · All Four · Named · Savored · Unsent · Future Self · Both Weathers · Turned Over.

7. **The family tones** (`config/families.json` + `assets/js/sound.js` (not yet written)). All in D major so any garden sounds like a chord: Joy F#5 · Calm A4 · Love B4 · Hope E5 · Sad D4 · Afraid G4 · Angry E4 · Tired A3 · quiet D5. Threshold chimes: root + fifth + octave, staggered 180 ms. Bloom: a three-note chord. Common Ground: two tones (A4, B4) resolving to one (D5). The bell: one struck D5 with a 4.5 s decay and two faint partials. All short, soft, never looping, off by default.
...
10. **The fifth practice**. **Return**: what I return to (thread, ago, tomorrow). Proposed on `/futures` (not a route in this tree). *Why:* the garden itself is made of returning; the four practices never named it.
