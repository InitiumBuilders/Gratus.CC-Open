...
151. **Taste that lives in a habit is lost the first busy day; taste that lives in a gate is not.** The instruction to take machine writing out before it ships is now a gate that fails the build, next to contrast, banned words and the open-source scrub.
152. **A writing gate must know whose words it is judging.** `config/his-words.json` lists August's lines and the gate looks away from every one of them. Without that registry the gate would be a machine for sanding the voice off the one part of this product that has one, and it would do it confidently.
153. **The decoy runs in both directions.** Every rule must still catch a planted violation, and his lines, which carry several of the shapes the gate hunts, must still come through untouched. A gate that has stopped biting is worse than no gate. A gate that has started biting the owner is worse than both.
